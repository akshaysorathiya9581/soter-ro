<div class="hr-title hr-long center"><abbr>@lang('web.workshops_title')</abbr> </div>
<section>
    <div class="col-md-8 col-md-offset-2">
        <p align="justify">
            @lang('web.workshops_intro')
        </p>
        <div class="col-md-8 col-md-offset-3">
            <ul align="justify">
                <li>@lang('web.workshops_advantage1')</li>
                <li>@lang('web.workshops_advantage2')</li>
                <li>@lang('web.workshops_advantage3')</li>
                <li>@lang('web.workshops_advantage4')</li>
            </ul>
        </div>
        <hr />
        </br>
        <div class="col-md-12">
            <div class="hr-title hr-short center"><abbr>@lang('web.workshops_active_title')</abbr></div>
        </div>

        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_consultancy')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_tax_news_2025')</h6>
                </div>
                <div class="col-md-2">
                    <a class="Photography-Button disabled" href="/atelier-consultanta">
                        <span><i class="fa fa-cloud"></i>@lang('web.workshops_button_details')</span>
                    </a>
                </div>
            </div>
        </div>
        <!--<div class="hr-title hr-short center"><abbr>No active courses</abbr></div> -->
        <hr />
        </br>
        <div class="hr-title hr-short center"><abbr>@lang('web.workshops_completed_title')</abbr></div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_consultancy')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>
                        @lang('web.workshops_course_tax_intentions_2018')
                    </h6>
                </div>
                <!--<div class="col-md-2">
                    <a class="Photography-Button disabled" href="/atelier-consultanta"><span><i class="fa fa-cloud"></i>Details</span></a>
                </div> -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_payroll')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>
                        @lang('web.workshops_course_payroll_legislation_2018_recap')
                    </h6>
                </div>
                <!-- <div class="col-md-2">
                    <a class="Photography-Button disabled" href="/atelier-salarizare"><span><i class="fa fa-cloud"></i>Details</span></a>
                </div> -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_consultancy')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>
                        @lang('web.workshops_course_tax_burden_2018')
                    </h6>
                </div>
                <!-- <div class="col-md-2">
                    <a class="Photography-Button disabled" href="/atelier-consultanta"><span><i class="fa fa-cloud"></i>Details</span></a>
                </div> -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_payroll')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>
                        @lang('web.workshops_course_payroll_burden_2018')
                    </h6>
                </div>

                <!--
                                <div class="col-md-2">
                                    <a class="Photography-Button disabled" href="/atelier-salarizare"><span><i class="fa fa-cloud"></i>Details</span></a>
                                </div>
            -->

            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_payroll')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>
                        @lang('web.workshops_course_payroll_contributions_transfer') <span
                            class="text-rotator" data-rotate-effect="fadeIn" data-rotate-speed="1000"
                            style="color:#26B8F3"> </span>
                    </h6>
                </div>
                <!--
                                <div class="col-md-2">
                                    <a class="Photography-Button disabled" href="/Servicii/Atelier2"><span><i class="fa fa-cloud"></i>Details</span></a>
                                </div>
            -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_tax')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_tax_split2') <span
                            class="text-rotator" data-rotate-effect="fadeIn" data-rotate-speed="1000"
                            style="color:#26B8F3"> </span> </h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="/Servicii/Atelier1"><span><i class="fa fa-cloud"></i>Details</span></a>
                                </div>
            -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_payroll')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_payroll_leg_changes_impact')</h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="/Servicii/Atelier2"><span><i class="fa fa-cloud"></i>Details</span></a>
                            </div>
            -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_tax')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_tax_split_payment')</h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="/Servicii/Atelier1"><span><i class="fa fa-cloud"></i>Details</span></a>
                            </div>
            -->
            </div>
        </div>
        <!-- <div class="jumbotron jumbotron-border" data-animation="fadeInDown" data-animation-delay="200" style="border-color:blue">-->
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_tax')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_tax_promotions_control')</h6>
                </div>
                <!-- <div class="col-md-2">
                    <a class="Photography-Button disabled" href="/ro/Servicii/Atelier1"><span><i class="fa fa-cloud"></i>Details</span></a>
                </div> -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_payroll')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_payroll_labor_law_practice')</h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="/atelier-salarizare"><span><i class="fa fa-cloud"></i>Details</span></a>
                            </div>-->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_accounting')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_accounting_f394')</h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="#"><span><i class="fa fa-cloud"></i>Completed</span></a>
                            </div>
            -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_payroll')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_payroll_labor_legislation')</h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="#"><span><i class="fa fa-cloud"></i>Completed</span></a>
                            </div>
            -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_payroll')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_payroll_income_tax')</h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="#"><span><i class="fa fa-cloud"></i>Completed</span></a>
                            </div>
            -->
            </div>
        </div>
        <div class="jumbotron jumbotron-border">
            <h4>@lang('web.workshops_category_tax')</h4>
            <hr />
            </br>
            <div class="col-md-12">
                <div class="col-md-10">
                    <h6>@lang('web.workshops_course_tax_fall_winter_2016')</h6>
                </div>
                <!--
                                <div class="col-md-2">
                                <a class="Photography-Button disabled" href="#"><span><i class="fa fa-cloud"></i>Completed</span></a>
                            </div>
            -->
            </div>
        </div>
    </div>
</section>

